...
import { MONTHS, monthName, monthShort, type ComparisonStatus } from "@/features/water/types";
...
/**
 * Water Consumption Summary Report — a full 12-month annual summary, one row
 ...
 * Mirrors the Energy Consumption annual report standard.
 */
export function WaterSummaryPage() {
  ...
  const accounts = useWaterAccounts(React.useMemo(() => ({}), []));
  ...
  const breakdown = React.useMemo(() => {
    ...
    return accounts.data
      .map((account) => {
        ...
        // Consumption as recorded by the meter readings, for comparison with
        // the billed figure. Null when no readings exist for the period.
        const meteredRows = subRows.map((r) =>
          meteredConsumption(readings.data, r.submeter.id, effectiveBreakdownMonth, year),
        );
        const anyMetered = meteredRows.some((m) => m !== null);
        return {
          account,
          rows: subRows,
          subtotalAmount: subRows.reduce((s, r) => s + (r.current ?? 0), 0),
          subtotalConsumption: subRows.reduce((s, r) => s + r.currentConsumption, 0),
          subtotalMetered: anyMetered
            ? meteredRows.reduce((s: number, m) => s + (m ?? 0), 0)
            : null,
        };
      })
      .filter((g) => g.rows.length > 0);
  }, [accounts.data, submeters.data, submeterBills.data, readings.data, effectiveBreakdownMonth, year]);

  const grandTotal = React.useMemo(() => {
    const anyMetered = breakdown.some((g) => g.subtotalMetered !== null);
    return {
      amount: breakdown.reduce((s, g) => s + g.subtotalAmount, 0),
      consumption: breakdown.reduce((s, g) => s + g.subtotalConsumption, 0),
      metered: anyMetered
        ? breakdown.reduce((s: number, g) => s + (g.subtotalMetered ?? 0), 0)
        : null,
  ...
  const fileStem = `water-consumption-${year}`;
  ...
  return (
    <PageTransition className="space-y-4">
      <div data-print-hide="">
        <PageHeader
          title="Water Consumption Summary Report"
          description="Twelve-month annual summary of water expense across all registered accounts."
          actions={
            <>
              <Button variant="ghost" onClick={() => navigate("/water")}>
                <ArrowLeft />
                Back
              </Button>
            </>
      ...
      <ContainerCard data-print-sheet="" className="mx-auto w-full max-w-[1400px] p-8">
        ...
        <div className="mt-5 overflow-x-auto">
          <div className="rounded-t-lg bg-neutral-900 px-3 py-2 text-[12px] font-semibold uppercase tracking-[0.05em] text-white">
            Monthly Water Consumption — {year}
          </div>
        ...
        <div className="mt-4 flex flex-wrap items-stretch gap-4 break-inside-avoid">
          <div className="grid flex-1 grid-cols-2 gap-x-8 gap-y-1.5 rounded-lg border border-neutral-200 p-4 text-[11.5px] sm:grid-cols-3">
            ...
            <div>
              <OverlineLabel>Total Annual Water Expense</OverlineLabel>
              ...
              <tbody>
                {breakdown.map((group) => (
                  <React.Fragment key={group.account.id}>
                    <tr>
                      ...
                      <td className="border border-neutral-200 bg-neutral-100/70 px-2.5 py-1.5 text-right tabular-nums font-semibold text-neutral-700">
                        {group.subtotalMetered === null
                          ? "—"
                          : group.subtotalMetered.toLocaleString("en-PH", {
                    ...
                    {group.rows.map((r) => (
                      <tr key={r.submeter.id}>
                        ...
                        <td className="border border-neutral-200 px-2.5 py-1.5 text-neutral-600">
                          {departmentByCode(r.submeter.assignedOffice)?.name ??
                            r.submeter.assignedOffice}
                          {(r.submeter.assignedDepartment || r.submeter.assignedFacility) && (
                        ...
                        <td className="border border-neutral-200 px-2.5 py-1.5 text-right tabular-nums text-neutral-600">
                          {(() => {
                            ...
                            return metered === null
                              ? "—"
                              : metered.toLocaleString("en-PH", { maximumFractionDigits: 0 });
                          })()}
                        </td>
                ...
                <tr>
                  ...
                  <td className="border border-neutral-200 bg-neutral-900 px-2.5 py-2 text-right tabular-nums font-bold text-white">
                    {grandTotal.metered === null
                      ? "—"
                      : grandTotal.metered.toLocaleString("en-PH", { maximumFractionDigits: 0 })}
                  </td>
                  <td className="border border-neutral-200 bg-neutral-900 px-2.5 py-2 text-right tabular-nums font-bold text-white">
